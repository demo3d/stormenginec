...
StormGLContext = function(width, height, loadScene) {
	...
	if(loadScene == undefined) {
		document.write(
				'<div>\n'+
					'<div id="DIVID_stormPreloader" style="position:absolute; width:\n'+this.viewportWidth+'px;margin-top:\n'+(this.viewportHeight-35)+'px; text-align:right;"><a href="http://code.google.com/p/stormenginec/" target="_blank"><img src="\n'+stormEngineC.includeDirectory+'/resources/stormenginec_logo.png" border="0"/></a></div>\n'+
					'<div id="stormStatus" style="position:absolute; width:\n'+this.viewportWidth+'px;margin-top:\n'+(this.viewportHeight/2)+'px; text-align:center; color:#EEE; font-weight:bold; text-shadow: 2px 2px #000; "></div>\n'+
					'<canvas id="stormCanvas" width="\n'+this.viewportWidth+'" height="\n'+this.viewportHeight+'"></canvas>\n'+
				'</div>'); 
	}
	this.g_targetCanvasId = document.getElementById("stormCanvas");
	try {
		this.gl = this.g_targetCanvasId.getContext("experimental-webgl");
	} catch(e) {
    }
	if(loadScene == undefined) {
		if(!this.gl) {
			document.write('Tu navegador no soporta WebGL. Descarga <a href="http://www.mozilla.com/">Firefox</a> o <a href="http://www.google.es/chrome">Chrome</a>');
		}
	}
	...
	if(loadScene == undefined) {
		...
		divDebug.style.left = "0px";
		divDebug.style.top = "0px";
		divDebug.style.textAlign = "left";
		divDebug.style.fontSize = "11px";
		divDebug.style.background = "#FFF";
		divDebug.style.color = "#000";
		this.g_targetCanvasId.parentNode.appendChild(divDebug);
	}
	...
	this.mPMatrix;
	this.mPOrthoMatrix;
	this.ambientColor = $V3([0.41,0.4,0.43]);
	...
	// VALORES CONTEXTO
	this.gl.clearColor(0.7, 0.75, 0.8, 1.0);
	...
	imageElement.textureObj = this.textureRandom;
	imageElement.ctx = this.gl;
	imageElement.onload = function() {		
		imageElement.ctx.bindTexture(imageElement.ctx.TEXTURE_2D, imageElement.textureObj);
		imageElement.ctx.texImage2D(imageElement.ctx.TEXTURE_2D, 0, imageElement.ctx.RGB, imageElement.ctx.RGB, imageElement.ctx.UNSIGNED_BYTE, imageElement);
		imageElement.ctx.texParameteri(imageElement.ctx.TEXTURE_2D, imageElement.ctx.TEXTURE_MAG_FILTER, imageElement.ctx.LINEAR);
		imageElement.ctx.texParameteri(imageElement.ctx.TEXTURE_2D, imageElement.ctx.TEXTURE_MIN_FILTER, imageElement.ctx.LINEAR);	
		imageElement.ctx.texParameteri(imageElement.ctx.TEXTURE_2D, imageElement.ctx.TEXTURE_WRAP_S, imageElement.ctx.MIRRORED_REPEAT);
		imageElement.ctx.texParameteri(imageElement.ctx.TEXTURE_2D, imageElement.ctx.TEXTURE_WRAP_T, imageElement.ctx.MIRRORED_REPEAT);			
		imageElement.ctx.bindTexture(imageElement.ctx.TEXTURE_2D, null);
	};
	imageElement.src = stormEngineC.includeDirectory+'/resources/noise32x32.jpg';
...
StormGLContext.prototype.renderShadowsNow = function(node, buffersObject, light) {
	...
	this.gl.vertexAttribPointer(this.textureCoordAttributeRT1, 2, this.gl.FLOAT, false, 0, 0);
...
StormGLContext.prototype.renderShadowLayer = function(currentLight) {
	...
	for(var n = 0; n < this.nodes.length; n++) {
	    for(var nb = 0; nb < this.nodes[n].buffersObjects.length; nb++) {			
			...
			this.gl.vertexAttribPointer(this.textureCoordAttributeShadowLayer, 2, this.gl.FLOAT, false, 0, 0);
 ...
 */
StormGLContext.prototype.addGLTexture = function(imageElement) {
	imageElement.ctx.pixelStorei(imageElement.ctx.UNPACK_FLIP_Y_WEBGL, true);
	imageElement.ctx.pixelStorei(imageElement.ctx.UNPACK_PREMULTIPLY_ALPHA_WEBGL, true);
	imageElement.ctx.bindTexture(imageElement.ctx.TEXTURE_2D, imageElement.textureObj);
	imageElement.ctx.texImage2D(imageElement.ctx.TEXTURE_2D, 0, imageElement.ctx.RGBA, imageElement.ctx.RGBA, imageElement.ctx.UNSIGNED_BYTE, imageElement);
	
	imageElement.ctx.texParameteri(imageElement.ctx.TEXTURE_2D, imageElement.ctx.TEXTURE_MAG_FILTER, imageElement.ctx.LINEAR);
	imageElement.ctx.texParameteri(imageElement.ctx.TEXTURE_2D, imageElement.ctx.TEXTURE_MIN_FILTER, imageElement.ctx.LINEAR_MIPMAP_NEAREST);
	imageElement.ctx.generateMipmap(imageElement.ctx.TEXTURE_2D);
