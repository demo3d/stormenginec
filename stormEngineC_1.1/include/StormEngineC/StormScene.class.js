		...
		if(req.readyState == 4) {
			...
			for(var n = 0; n < lines.length; n++) {
				...
				if(lines[n].match(/^textureKdName /gi) != null) {
					bObject.attachTexture(req.textureDir+'/'+lines[n].replace(/^textureKdName /gi, ''), undefined, 'map_kd');
				...
				if(lines[n].match(/^textureBumpName /gi) != null) {
					if(lines[n].replace(/^textureBumpName /gi, '') != 'undefined') {
						bObject.attachTexture(req.textureDir+'/'+lines[n].replace(/^textureBumpName /gi, ''), undefined, 'map_bump');
