 ...
 */
 // ENVIA ESCENA A DEVICE TRIANGULO x TRIANGULO, MENOS PROCESADOS DEL COLOR POR CPU EN CADA SAMPLE (+RAPIDO)
StormRender = function(DIVID,callback) {
	...
	*/
	...
	for(var nL = 0; nL < this.lights.length; nL++) {
		this.clKernel_setZero.setKernelArg(nL+14, this.buffShadowNearDistance[nL]);
	}
...
StormRender.prototype.argumentsSetZero = function() {
	...
	var coma = ',';
	for(var nL = 0; nL < this.lights.length; nL++) {
		if(nL == this.lights.length-1) coma = '';
		str += '__global float* ShadowNearDistance'+nL+coma;
	}
	return str;
};
...
StormRender.prototype.updateObjects = function() {
	...
	for(var n = 0; n < this.nodes.length; n++) {
		for(var nb = 0; nb < this.nodes[n].buffersObjects.length; nb++) {
			...
			this.arrayBuffersTextures[numNB] = clContext.createImage2D(WebCL.CL_MEM_READ_ONLY, format,this.nodes[n].buffersObjects[nb].imageElement_Kd.width, this.nodes[n].buffersObjects[nb].imageElement_Kd.height, 0);	
			clCmdQueue.enqueueWriteImage(this.arrayBuffersTextures[numNB], true, [0,0,0], [this.nodes[n].buffersObjects[nb].imageElement_Kd.width,this.nodes[n].buffersObjects[nb].imageElement_Kd.height,1], 0, 0, this.nodes[n].buffersObjects[nb].arrayTEX_Kd, []);
			numNB++;
		}
	}
	for(var n = 0; n < this.lights.length; n++) {
		for(var nb = 0; nb < this.lights[n].buffersObjects.length; nb++) {
			...
			clCmdQueue.enqueueWriteImage(this.arrayBuffersTextures[numNB], true, [0,0,0], [1,1,1], 0, 0, this.lights[n].buffersObjects[nb].arrayTEX_Kd, []);
			numNB++;
		}
		//this.arrayBuffersTexturesLightRay[n] = clContext.createImage2D(WebCL.CL_MEM_READ_ONLY, format,this.lights[n].buffersObjects[0].imageElement_Kd.width, this.lights[n].buffersObjects[0].imageElement_Kd.height, 0);
		//clCmdQueue.enqueueWriteImage(this.arrayBuffersTexturesLightRay[n], true, [0,0,0], [this.lights[n].buffersObjects[0].imageElement_Kd.width,this.lights[n].buffersObjects[0].imageElement_Kd.height,1], 0, 0, this.lights[n].buffersObjects[0].arrayTEX_Kd, []);
...
StormRender.prototype.setCam = function(nodeCam) {
	...
	this.clKernel_setZero.setKernelArg(0, nodeCam.nodeGoal.mWMatrix.e[3], WebCL.types.FLOAT);
	this.clKernel_setZero.setKernelArg(1, nodeCam.nodeGoal.mWMatrix.e[7], WebCL.types.FLOAT);
	this.clKernel_setZero.setKernelArg(2, nodeCam.nodeGoal.mWMatrix.e[11], WebCL.types.FLOAT);
	this.clKernel_setZero.setKernelArg(3, nodeCam.nodePivot.mWMatrix.e[3], WebCL.types.FLOAT);
	this.clKernel_setZero.setKernelArg(4, nodeCam.nodePivot.mWMatrix.e[7], WebCL.types.FLOAT);
	this.clKernel_setZero.setKernelArg(5, nodeCam.nodePivot.mWMatrix.e[11], WebCL.types.FLOAT);
	...
	for(var n = 0; n < this.nodes.length; n++) {
		for(var nb = 0; nb < this.nodes[n].buffersObjects.length; nb++) {
			...
			this.clKernel_PrimaryRays.setKernelArg(24, this.nodes[n].buffersObjects[nb].Ns, WebCL.types.FLOAT);
	...
	for(var n = 0; n < this.lights.length; n++) {
		for(var nb = 0; nb < this.lights[n].buffersObjects.length; nb++) {
			...
			this.clKernel_PrimaryRays.setKernelArg(24, this.lights[n].buffersObjects[nb].Ns, WebCL.types.FLOAT);
			this.clKernel_PrimaryRays.setKernelArg(46, this.arrayBuffersTextures[numbNB]);
			for(var b = 0; b < this.lights[n].buffersObjects[nb].nodeMeshIndexArray.length/3; b++) {
	...
	for(var currBounce = 1; currBounce <= this.MAXBOUNCES; currBounce++) {
		...
		for(var n = 0; n < this.nodes.length; n++) {
			...
			for(var nb = 0; nb < this.nodes[n].buffersObjects.length; nb++) {
				...
				this.clKernel_SecundaryRays.setKernelArg(27, this.nodes[n].buffersObjects[nb].Ns, WebCL.types.FLOAT);
		...
		for(var n = 0; n < this.lights.length; n++) {
			...
			for(var nb = 0; nb < this.lights[n].buffersObjects.length; nb++) {
				...
				this.clKernel_SecundaryRays.setKernelArg(27, this.lights[n].buffersObjects[nb].Ns, WebCL.types.FLOAT);
				this.clKernel_SecundaryRays.setKernelArg(59, this.arrayBuffersTextures[numbNB]);
				for(var b = 0; b < this.lights[n].buffersObjects[nb].nodeMeshIndexArray.length/3; b++) {
