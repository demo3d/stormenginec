...
StormGraph = function(sec, jsonIn) { StormNode.call(this);
	...
	// mesh to use
	this.nodeArrow = new StormNode(this._sec);
	this.nodeArrow.loadTriangle();
	...
	var str_posOpposite = this.source_positionOpposite();
	...
	// ARROWS
	//******************************
	this.clglWork_arrows = this.webCLGL.createWork(this.offset);
	
	//VERTEX AND FRAGMENT PROGRAMS	
	this.vfProgram_arrows = this.webCLGL.createVertexFragmentProgram();
	this.vfProgram_arrows.setVertexSource(str_vfp[1][0], str_vfp[0][0]);
	this.vfProgram_arrows.setFragmentSource(str_vfp[3][0], str_vfp[2][0]);
	
	// VERTEX AND FRAGMENT PROGRAMS	NORMALS AND DEPTH
	//this.vfProgram_ND_arrows = this.webCLGL.createVertexFragmentProgram();
	//this.vfProgram_ND_arrows.setVertexSource(str_vfp_nd[1][0], str_vfp_nd[0][0]);
	//this.vfProgram_ND_arrows.setFragmentSource(str_vfp_nd[3][0], str_vfp_nd[2][0]);
	
	// KERNEL POSITION BY DIRECTION
	this.kernel_positionByDirection_arrows = this.webCLGL.createKernel();
	this.kernel_positionByDirection_arrows.setKernelSource(str_posdir);
	
	// KERNEL POSITION OPPOSITE
	this.kernel_positionOpposite_arrows = this.webCLGL.createKernel();
	this.kernel_positionOpposite_arrows.setKernelSource(str_posOpposite);
	
	// KERNEL DIRECTION
	this.kernel_direction_arrows = this.webCLGL.createKernel(); 
	this.kernel_direction_arrows.setKernelSource(str_dir);	
	
	// ADD TO WORK
	this.clglWork_arrows.addVertexFragmentProgram(this.vfProgram_arrows, "graph");
	//this.clglWork_arrows.addVertexFragmentProgram(this.vfProgram_ND_arrows, "graph_nd");
	this.clglWork_arrows.addKernel(this.kernel_positionByDirection_arrows, "posXYZW");
	this.clglWork_arrows.addKernel(this.kernel_positionOpposite_arrows, "posXYZW_opposite");
	this.clglWork_arrows.addKernel(this.kernel_direction_arrows, "dir");
	
	this.arrowDrawMode = 4; // 4 TRIANGLES
	...
	this.splitArrowsEvery = parseInt((256*256)/3); // 3 arrow indices
...
/** @private **/
StormGraph.prototype.source_positionOpposite = function() {
...
StormGraph.prototype.updateLinks = function() {
	...
	this.clglWork_links.setArg("selfShadows", 0.0);
...
* @private
 */
StormGraph.prototype.addArrowNow = function(jsonIn) {
	var oppositeId = 0;
	for(var o=0; o < 2; o++) {
		
		for(var i=0; i < this.nodeArrow.buffersObjects.length; i++) {
			var bo = this.nodeArrow.buffersObjects[i];
			for(var n=0; n < bo.nodeMeshVertexArray.length/3; n++) {
				var idxVertex = n*3;
				...
				this.arrayArrowVertexPos.push(bo.nodeMeshVertexArray[idxVertex], bo.nodeMeshVertexArray[idxVertex+1], bo.nodeMeshVertexArray[idxVertex+2], 1.0);
				this.arrayArrowVertexNormal.push(bo.nodeMeshNormalArray[idxVertex], bo.nodeMeshNormalArray[idxVertex+1], bo.nodeMeshNormalArray[idxVertex+2], 1.0);
				//console.log(bo.nodeMeshVertexArray[idxVertex]);
				if(o == 0) {
		...
		for(var i=0; i < this.nodeArrow.buffersObjects.length; i++) {
			...
			for(var n=0; n < bo.nodeMeshIndexArray.length; n++) {
				...
				this.arrayArrowIndices.push(this.startIndexId_arrow+bo.nodeMeshIndexArray[idxIndex]);
				//console.log(this.startIndexId+bo.nodeMeshIndexArray[idxIndex]);
				
				if(bo.nodeMeshIndexArray[idxIndex] > maxArrowIndexId) {
					maxArrowIndexId = bo.nodeMeshIndexArray[idxIndex];			
				}
			}
		}
 ...
 * Update arrows
 */
StormGraph.prototype.updateArrows = function() {
	this.updateForcesAndPP(this.clglWork_arrows);
	
	this.clglWork_arrows.setArg("idx", this.arrayArrowId, this.splitArrows); 
	this.clglWork_arrows.setArg("nodeId", this.arrayArrowNodeId, this.splitArrows);
	...
	this.clglWork_arrows.setArg("posXYZW", this.arrayArrowPosXYZW, this.splitArrows);
	this.clglWork_arrows.setArg("posXYZW_opposite", this.arrayArrowPosXYZW_opposite, this.splitArrows);	 
	this.clglWork_arrows.setArg("oppositeId", this.arrayArrow_oppositeId, this.splitArrows);	
	this.clglWork_arrows.setArg("isArrow", 1.0);
	this.clglWork_arrows.setArg("bufferWidth", this.clglWork_arrows.buffers["posXYZW"].items[0].W);
	this.clglWork_arrows.setArg("bufferHeight", this.clglWork_arrows.buffers["posXYZW"].items[0].H);
	 
	this.clglWork_arrows.setArg("initPos", this.arrayArrowPosXYZW, this.splitArrows);
	this.clglWork_arrows.setArg("nodeVertexPos", this.arrayArrowVertexPos, this.splitArrows);
	this.clglWork_arrows.setArg("nodeVertexNormal", this.arrayArrowVertexNormal, this.splitArrows);
	this.clglWork_arrows.setArg("nodeVertexCol", this.arrayArrowVertexColor, this.splitArrows);
	this.clglWork_arrows.setIndices(this.arrayArrowIndices, this.splitArrowIndices);
	
	this.arrayArrowDir = [];	
	for(var n=0; n < this.arrayArrowId.length; n++) {
		this.arrayArrowDir.push(0, 0, 0, 255);
	}
	this.clglWork_arrows.setArg("dir", this.arrayArrowDir, this.splitArrows);
	this.clglWork_arrows.setArg("initDir", this.arrayArrowDir, this.splitArrows);
	
	this.arrayArrowPolaritys = [];	
	for(var n=0; n < this.arrayArrowId.length; n++) {
		this.arrayArrowPolaritys.push(1);
	}
	this.clglWork_arrows.setArg("particlePolarity", this.arrayArrowPolaritys, this.splitArrows);
	
	this.arrayArrowDestination = [];	
	for(var n=0; n < this.arrayArrowId.length; n++) {
		this.arrayArrowDestination.push(0, 0, 0, 255);
	}
	this.clglWork_arrows.setArg("dest", this.arrayArrowDestination, this.splitArrows);
	
	this.clglWork_arrows.setArg("PMatrix", this._sec.defaultCamera.mPMatrix.transpose().e);
	this.clglWork_arrows.setArg("cameraWMatrix", this._sec.defaultCamera.MPOS.transpose().e);
	this.clglWork_arrows.setArg("nodeWMatrix", this.MPOS.transpose().e);
	this.clglWork_arrows.setArg("nodesSize", this.currentArrowId-1);
	this.clglWork_arrows.setArg("sunPos", [this._sec.lights[0].direction.e[0], this._sec.lights[0].direction.e[1], this._sec.lights[0].direction.e[2], 1.0]);
	this.clglWork_arrows.setArg("selfShadows", 0.0);
	this.clglWork_arrows.setArg("ambientColor", [this._sec.getAmbientColor()[0], this._sec.getAmbientColor()[1], this._sec.getAmbientColor()[2], 1.0]);
	
	this.clglWork_arrows.setArg("enableDestination", this.enDestination);
	this.clglWork_arrows.setArg("destinationForce", this.destinationForce);
	this.clglWork_arrows.setArg("lifeDistance", this.lifeDistance);
	this.clglWork_arrows.setArg("pointSize", this.pointSize);
	this.clglWork_arrows.setArg("enableDrag", 0);
	this.clglWork_arrows.setArg("idToDrag", 0);
	this.clglWork_arrows.setArg("MouseDragTranslationX", 0);
	this.clglWork_arrows.setArg("MouseDragTranslationY", 0);
	this.clglWork_arrows.setArg("MouseDragTranslationZ", 0);
	this.clglWork_arrows.setArg("islink", 1);
}; 
...
StormGraph.prototype.prerender = function() {
	...
	if(this.arrayArrowId.length > 0) this.clglWork_arrows.enqueueNDRangeKernel();
};
/**
...
StormGraph.prototype.render_ND = function() {
	if(this.arrayNodeId.length > 0) {
		this.clglWork_nodes.enqueueVertexFragmentProgram(undefined, "graph_nd", (function() {
	...
	/*if(this.arrayLinkId.length > 0) {
		this.clglWork_links.enqueueVertexFragmentProgram(undefined, "graph_nd", (function() {
			//this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
			if(this._sec.stormGLContext.view_Normals) {
				this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
			} else {
	...
	if(this.arrayArrowId.length > 0) {
		this.clglWork_arrows.enqueueVertexFragmentProgram(undefined, "graph_nd", (function() {
			//this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
			if(this._sec.stormGLContext.view_Normals) {
				this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
			} else {
			...
			this.clglWork_arrows.setArg("cameraWMatrix", this._sec.defaultCamera.MPOS.transpose().e);
			this.clglWork_arrows.setArg("nodeWMatrix", this.MPOS.transpose().e);
			this.clglWork_arrows.setArg("uFar", this._sec.stormGLContext.far);
		}).bind(this), this.arrowDrawMode);
	}*/
};
/**
* Make render
...
StormGraph.prototype.render = function() {
	if(this.arrayNodeId.length > 0) {
		this.clglWork_nodes.enqueueVertexFragmentProgram(undefined, "graph", (function() {
			//this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
	...
	if(this.arrayLinkId.length > 0) {
		this.clglWork_links.enqueueVertexFragmentProgram(undefined, "graph", (function() {
			//this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
			if(this._sec.stormGLContext.view_SceneNoDOF || this._sec.defaultCamera.DOFenable == false) {
				this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
			} else {
	...
	if(this.arrayArrowId.length > 0) {
		this.clglWork_arrows.enqueueVertexFragmentProgram(undefined, "graph", (function() {
			//this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
			if(this._sec.stormGLContext.view_SceneNoDOF || this._sec.defaultCamera.DOFenable == false) {
				this.gl.bindFramebuffer(this.gl.FRAMEBUFFER, null);
			} else {
			...
			this.clglWork_arrows.setArg("cameraWMatrix", this._sec.defaultCamera.MPOS.transpose().e);
			this.clglWork_arrows.setArg("nodeWMatrix", this.MPOS.transpose().e);
			this.clglWork_arrows.setArg("nodesSize", parseFloat(this.currentNodeId-1));
		}).bind(this), this.arrowDrawMode);
	}
};
...
StormGraph.prototype.setSelfShadows = function(bselfShadows) {
	...
	this.clglWork_arrows.setArg("selfShadows", ((this.selfShadows == true)?1.0:0.0));
};
...
StormGraph.prototype.set_polarity = function(arr) {
	...
	this.clglWork_nodes.setArg("particlePolarity", this.arrayNodePolaritys, this.splitNodes);
	this.clglWork_links.setArg("particlePolarity", this.arrayLinkPolaritys, this.splitLinks);
	this.clglWork_arrows.setArg("particlePolarity", this.arrayLinkPolaritys, this.splitLinks);
};
...
* @type Void
*/
StormGraph.prototype.setArrowsDestinationToNodesDestination = function() {
...
* @type Void
*/
StormGraph.prototype.setArrowsDirToNodesDir = function() {
	this.arrayArrowDir = [];	
	for(var n=0; n < this.arrayArrowId.length; n++) {
		var currentArrowNodeName = this.arrayArrowNodeName[n];		
		var nodeNameItemStart = this._nodesByName[currentArrowNodeName].itemStart;
 ...
 * setLinksPosToNodesPos
 */
StormGraph.prototype.setArrowsPosToNodesPos = function() {
	this.arrayArrowPosXYZW = [];	
	for(var n=0; n < this.arrayArrowId.length; n++) {
		var currentArrowNodeName = this.arrayArrowNodeName[n];		
		var nodeNameItemStart = this._nodesByName[currentArrowNodeName].itemStart;
